...
import { BookOpen, Trash2, RefreshCw, Eye, Edit3, CheckCircle, Clock, AlertCircle } from 'lucide-react';
...
export const ChapterList: React.FC<ChapterListProps> = ({ workId, onChapterClick }) => {
  ...
  const handleDelete = async (chapterNumber: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      // 从本地状态中移除
      setChapters(prev => prev.filter(ch => ch.chapterNumber !== chapterNumber));
    } catch (err) {
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      ...
      case 'generating':
        return { icon: Clock, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-900/20' };
      case 'failed':
        return { icon: AlertCircle, color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/20' };
      default:
        return { icon: Clock, color: 'text-gray-500', bg: 'bg-gray-50 dark:bg-gray-800' };
    }
  };
            ...
            {chapter.status === 'generating' && (
              <div className="absolute inset-0 bg-white/50 dark:bg-slate-900/50 flex items-center justify-center rounded-xl">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-primary">生成中...</span>
                </div>
              </div>
            )}
          </div>
