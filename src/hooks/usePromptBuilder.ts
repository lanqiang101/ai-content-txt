...
### 节奏
- 平均段落长度：${rhythm.averageParaLength}
- 冲突频率：${rhythm.conflictFrequency}%
...
`;
    ...
    if (antiAI.templateDeletePercent > 0) {
      prompt += `- 删除模板化句式，删除"只见""就在这时""殊不知"这类AI常用开头\n`;
    }
    if (antiAI.unpredictableTurnPercent > 20) {
